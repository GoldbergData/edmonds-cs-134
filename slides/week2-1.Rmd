---
title: "CS 134 Data Visualization: Week 2, Session 3, Mapping data to graphics"
author: "Joshua Goldberg <br><span style='font-size: 50%;'>Edmonds College<br>Thank you Andrew Heiss of Georgia State University for major parts of these slides</span>"
date: "`r Sys.Date()`"
output:
...
minard_temps <- read_csv("../slides/data/Minard.temp.csv") %>% 
  uncount(days) %>% 
...
```{r show-ggplot-template, echo=FALSE, tidy=FALSE, include=FALSE}
decorate('
ggplot(data = DATA) +
...
```{r ggplot-template-example, echo=FALSE, include=FALSE, eval=FALSE}
decorate('
ggplot(data = troops) +
...
# https://github.com/vincentarelbundock/Rdatasets/tree/master/csv/HistData
troops <- read_csv("../slides/data/Minard.troops.csv") %>% 
...
```{r ggplot-template-example1, echo=FALSE, include=FALSE, eval=FALSE}
decorate('
ggplot(data = troops) +
  geom_path(mapping = aes(x = longitude,
                          y = latitude,
...
```{r gapminder-example, echo=FALSE, include=FALSE, eval=FALSE}
decorate('
